  ...
  group('BufferSurface', () {
    group('drawText', () {
      test('writes a run starting at the given cell', () {
        final b = _buf(10, 1);
        BufferSurface(b).drawText(2, 0, 'hi', const Style());
      ...
      test('redeems the carried style onto each cell', () {
        final b = _buf(5, 1);
        BufferSurface(b).drawText(0, 0, 'ok', const Style(fg: Color.green, bg: Color.red));
        final c = b[(x: 0, y: 0)];
      ...
      test('clamps a run past the right edge instead of throwing', () {
        ...
        expect(() => s.drawText(1, 0, 'abcdef', const Style()), returnsNormally);
      ...
      test('drops a run on a row outside the buffer', () {
        final b = _buf(5, 2);
        expect(() => BufferSurface(b).drawText(0, 5, 'x', const Style()), returnsNormally);
        expect(b[(x: 0, y: 0)].symbol, ' ');
      });

      test('a wide glyph fills two cells, the trailing one skipped', () {
        final b = _buf(4, 1);
        BufferSurface(b).drawText(0, 0, '🦀', const Style());
        expect(b[(x: 0, y: 0)].symbol, '🦀');
      ...
      test('a wide glyph that would straddle the right edge is dropped', () {
        ...
        BufferSurface(b).drawText(2, 0, '🦀', const Style());
    ...
    group('fillRect', () {
      test('fills every cell of the rect with the style', () {
        final b = _buf(4, 3);
        BufferSurface(b).fillRect(const plume.Rect(1, 1, 2, 2), const Style(bg: Color.blue));
      ...
      test('clamps a rect that spills past the buffer instead of throwing', () {
        ...
        final s = BufferSurface(b);
        expect(
          () => s.fillRect(const plume.Rect(1, 1, 10, 10), const Style(bg: Color.blue)),
          returnsNormally,
        );
    ...
    group('clipping', () {
      test('drops a fill entirely outside the clip', () {
        ...
        BufferSurface(b)
          ...
          ..fillRect(const plume.Rect(5, 0, 3, 3), const Style(bg: Color.blue))
          ..popClip();
      ...
      test('trims a fill to the clip', () {
        ...
        BufferSurface(b)
          ...
          ..fillRect(const plume.Rect(2, 0, 6, 1), const Style(bg: Color.blue))
          ..popClip();
      ...
      test('trims a text run that starts left of the clip', () {
        ...
        BufferSurface(b)
          ...
          ..drawText(0, 0, 'abcdefgh', const Style())
          ..popClip();
    ...
    test('drawBorder is deferred to the Block port and throws', () {
      final b = _buf(5, 5);
      expect(
        () => BufferSurface(b).drawBorder(const plume.Rect(0, 0, 5, 5), const Style()),
