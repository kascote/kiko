...
- `RadioGroup` / `RadioGroupModel` — one exclusive choice among options; emits `RadioChangeEvent`; radio glyph presets on `CheckGlyphs`
...
- A composite scopes its part's result with `scopeUnder`; a part arms and asks with its bare id (`docs/components.md`).
- The request arm has no guard and its fetch ends in `declineLoad` (`docs/async-loading.md`).
