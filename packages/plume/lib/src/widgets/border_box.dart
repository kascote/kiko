...
/// Which horizontal edge of a [BorderBox] a label rides on.
///
...
/// A label riding on one edge of a [BorderBox].
///
...
/// A single-child box that fills a background, draws a uniform border, and
/// carries labels on its top and bottom edges.
///
/// The box is a decorated frame around one [child]. Layout insets the child by
/// the border — one cell on each side when [border] is present — plus any
/// [padding]; paint then fills the [background], strokes the [border], draws the
/// child, and lays each of [labels] over the border on its edge. Every
/// appearance — the border glyphs and colour, the fill — rides in an opaque
/// paint token the box never inspects; only the surface reads it.
///
/// The border is uniform: one token strokes all four sides at once. A box with
/// no [border] still reserves a top or bottom row for any label on that edge, so
/// a titled but borderless box insets its child the same way a bordered one does.
class BorderBox<T> extends RenderNode<T> {
  /// Wraps [child] with an optional [border], [padding], [background] fill and
  /// edge [labels].
  BorderBox({
    required this.child,
    this.border,
  ...
  Size performLayout(BoxConstraints constraints, LayoutContext context) {
    ...
    final childSize = child.layout(constraints.deflate(horizontal, vertical), context);
    child.offset = Offset(left, top);
    final box = constraints.constrain(Size(childSize.w + horizontal, childSize.h + vertical));

    _layoutLabels(context, box, edge);
    return box;
  }
