...
void main() {
  group('Block', () {
    ...
    test('has title at position takes into account all positioning declarations', () {
      var block = Block();
    ...
    test('vertical space takes into account padding', () {
      final cases = [
        ...
        (border: Borders.none, pad: const Padding(top: 10, left: 100, bottom: 13, right: 100), expected: (10, 13)),
        (border: Borders.all, pad: const Padding(top: 1, left: 100, bottom: 3, right: 100), expected: (2, 4)),
      ];
    ...
    test('vertical space takes into account borders and title', () {
      final cases = [
        (block: Block(), borders: Borders.top, pos: TitlePosition.top, vs: (1, 0)),
        (block: Block(), borders: Borders.right, pos: TitlePosition.top, vs: (1, 0)),
        (block: Block(), borders: Borders.bottom, pos: TitlePosition.top, vs: (1, 1)),
        (block: Block(), borders: Borders.left, pos: TitlePosition.top, vs: (1, 0)),
        (block: Block(), borders: Borders.top, pos: TitlePosition.bottom, vs: (1, 1)),
        (block: Block(), borders: Borders.right, pos: TitlePosition.bottom, vs: (0, 1)),
        (block: Block(), borders: Borders.bottom, pos: TitlePosition.bottom, vs: (0, 1)),
        (block: Block(), borders: Borders.left, pos: TitlePosition.bottom, vs: (0, 1)),
      ];
    ...
    test('horizontal space takes into_account padding', () {
      var block = Block(padding: const Padding(top: 100, left: 1, bottom: 100, right: 1));
      expect(block.horizontalSpace(), (1, 1));
  ...
  test('horizontal space takes into account borders and padding', () {
    final cases = [
      (block: Block(borders: Borders.all), pad: const Padding(top: 1, left: 1, bottom: 1, right: 1), hs: (2, 2)),
      (block: Block(borders: Borders.all), pad: const Padding(left: 1), hs: (2, 1)),
      (block: Block(borders: Borders.all), pad: const Padding(right: 1), hs: (1, 2)),
      (block: Block(borders: Borders.all), pad: const Padding(top: 1), hs: (1, 1)),
      (block: Block(borders: Borders.all), pad: const Padding(bottom: 1), hs: (1, 1)),
      (block: Block(borders: Borders.left), pad: const Padding(left: 1), hs: (2, 0)),
      (block: Block(borders: Borders.left), pad: const Padding(right: 1), hs: (1, 1)),
      (block: Block(borders: Borders.right), pad: const Padding(right: 1), hs: (0, 2)),
      (block: Block(borders: Borders.right), pad: const Padding(left: 1), hs: (1, 1)),
    ];
  ...
  test('title content style', () {
    for (final align in Alignment.values) {
      ...
      Block()
        ..title(
          Line(content: 'test', style: const Style(fg: Color.yellow), alignment: align),
          TitlePosition.top,
        )
        ..render(buffer.area, buffer);
      expect(buffer.eq(Buffer.fromLines([Line(content: 'test', style: const Style(fg: Color.yellow))])), isTrue);
    }
  });

  test('border style', () {
    final buffer = Buffer.empty(Rect.create(x: 0, y: 0, width: 10, height: 3));
    Block(borders: Borders.all, borderStyle: const Style(fg: Color.yellow))
      ..titleTop(Line(content: 'test'))
      ..render(buffer.area, buffer);
    ...
    final expected = Buffer.fromStringLines([
      ...
      ..setStyle(Rect.create(x: 0, y: 0, width: 10, height: 3), const Style(fg: Color.yellow))
      ..setStyle(Rect.create(x: 1, y: 1, width: 8, height: 1), const Style(fg: Color.reset));
  ...
  test('render rounded border', () {
    final buffer = Buffer.empty(Rect.create(x: 0, y: 0, width: 10, height: 3));
    Block(borders: Borders.all, borderType: BorderType.rounded).render(buffer.area, buffer);
  ...
  test('render double border', () {
    final buffer = Buffer.empty(Rect.create(x: 0, y: 0, width: 10, height: 3));
    Block(borders: Borders.all, borderType: BorderType.double).render(buffer.area, buffer);
  ...
  test('render quadrant inside ', () {
    final buffer = Buffer.empty(Rect.create(x: 0, y: 0, width: 10, height: 3));
    Block(borders: Borders.all, borderType: BorderType.quadrantInside).render(buffer.area, buffer);
  ...
  test('render quadrant outside ', () {
    final buffer = Buffer.empty(Rect.create(x: 0, y: 0, width: 10, height: 3));
    Block(borders: Borders.all, borderType: BorderType.quadrantOutside).render(buffer.area, buffer);
  ...
  test('render solid border', () {
    final buffer = Buffer.empty(Rect.create(x: 0, y: 0, width: 10, height: 3));
    Block(borders: Borders.all, borderType: BorderType.thick).render(buffer.area, buffer);
  ...
  test('throw error if set type custom but provide no definition', () {
    final buffer = Buffer.empty(Rect.create(x: 0, y: 0, width: 10, height: 3));
    expect(
      () => Block(borders: Borders.all, borderType: BorderType.custom).render(buffer.area, buffer),
      throwsArgumentError,
    );
  });

  test('throw error if try to acces a BorderType no defined', () {
    expect(() => BorderType.custom.symbols(BorderType.custom), throwsArgumentError);
  });
}
